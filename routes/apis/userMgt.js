...
router.get('/loginexists', async (reqe, res, next) => {
    ...
    let rs = 'true';
    try {
        let sUser = await User.findOne({ "username": reqe.query.login }).lean().select({ "username": 1 });
...
router.post('/users', async (reqe, res, next) => {
    try {
        ...
        let sUser = await User.findOne({ "username": rawNewUser.username }).lean().select({ "username": 1 });
...
router.patch('/users/:id', async (reqe, res, next) => {
    try {
        ...
        let rsObj = { ok: "User has been updated.", id: doc._id };
...
router.delete('/users', async (reqe, res, next) => {
    try {
        ...
        reqe.body.forEach(async function(deleteObj) {
            let doc = await User.findOneAndUpdate({ "_id": deleteObj._id, "delFlag": false }, delObj);
            deleteId.push(doc._id);
            logger.audit("User Mgt", "Delete", doc._id, user.id, `User has been deleted by ${user.displayName}`);
        });
        
        let rsObj = { ok: "User has been deleted.", id: deleteId};
        res.json(rsObj);
...
router.patch('/roles/:id', async (reqe, res, next) => {
    try {
        ...
        let rsObj = { ok: "User Role has been Updated.", id: doc._id };
...
router.delete('/roles/:id', async (reqe, res, next) => {
    try {
        ...
        //save role 
        let delObj = { updatedBy: user._id, delFlag: true };
        let doc = await UserRole.findOneAndUpdate({ "_id": reqe.params.id, "delFlag": false }, delObj);
        let rsObj = { ok: "User Role has been deleted.", id: doc._id };
